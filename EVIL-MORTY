--==================================================
-- EVIL MORTY PORTAL GUN (БЕЗ ЗВУКОВ)
-- FULL LOCAL SCRIPT
--
-- ПОРТАЛЫ:
--   - 2 портала (A↔B)
...
--   - 3 фиксированных цвета: зелёный, жёлтый, синий
--   - Улучшенный Bloom
--
-- GUI:
--   - Горизонтальный, сверху
--   - Иконки режимов (P, L, C)
--   - Динамический цвет панели и бургера
--   - Сворачиваемое меню
--   - 3 кнопки выбора цвета
--
-- УПРАВЛЕНИЕ:
--   - Q – сброс порталов
--   - Esc – отмена выбора точки (в режиме POINT)
--   - Alt – скрыть/показать GUI
--   - F1 – полная выгрузка скрипта
--   - Кулдаун между выстрелами 0.3 сек
--==================================================
...
--==================================================
-- НАСТРОЙКИ (НАЧАЛЬНЫЙ ЦВЕТ - ЖЁЛТЫЙ)
--==================================================

local PORTAL_TEXTURE = "rbxassetid://77878374203347"

-- Динамические цвета портала
local portalColor = Color3.fromRGB(255, 195, 35) -- жёлтый
...
--==================================================
-- МОДЕЛЬ ПУШКИ (ОРИГИНАЛЬНАЯ EVIL MORTY)
--==================================================
...
--==================================================
-- CREATE GUN (ОРИГИНАЛЬНАЯ МОДЕЛЬ, БЕЗ ЗВУКА)
--==================================================
...
--==================================================
-- GUI (ГОРИЗОНТАЛЬНЫЙ, СВЕРХУ, БЕЗ ПОЛЗУНКА)
--==================================================
...
frame.Size = UDim2.new(0, 700, 0, 155)
frame.Position = UDim2.new(0.5, -350, 0, 20)
frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
frame.BackgroundTransparency = 0.5
frame.BorderSizePixel = 2
frame.BorderColor3 = portalColor
frame.Parent = gui
...
-- Кнопка-бургер (сворачивание в иконку)
local menuToggle = Instance.new("TextButton")
menuToggle.Name = "MenuToggle"
menuToggle.Size = UDim2.new(0, 50, 0, 50)
menuToggle.Position = UDim2.new(0, 15, 1, -65)
menuToggle.BackgroundColor3 = portalColorDark
menuToggle.BorderSizePixel = 2
menuToggle.BorderColor3 = portalColor
menuToggle.TextColor3 = portalColorLight
menuToggle.Text = "✕"
...
menuToggle.Activated:Connect(function()
    ...
    if menuOpen then
        ...
        menuToggle.Position = UDim2.new(0, 10, 1, -70)
    end
end)

-- Функция для создания кнопок-иконок (круглые с буквой)
local function createIconButton(text, x, y, bgColor, letter)
...
-- Кнопки режимов (иконки)
local pointButton = createIconButton("POINT", 10, 10, Color3.fromRGB(180, 125, 10), "P")
local playerButton = createIconButton("PLAYER", 70, 10, Color3.fromRGB(180, 125, 10), "L")
local placeButton = createIconButton("PLACE", 130, 10, Color3.fromRGB(180, 125, 10), "C")

-- Поле ввода
local input = Instance.new("TextBox")
input.Size = UDim2.new(0, 200, 0, 38)
input.Position = UDim2.new(0, 190, 0, 16)
...
fireButton.Position = UDim2.new(0, 400, 0, 12)
...
resetButton.Size = UDim2.new(0, 80, 0, 38)
resetButton.Position = UDim2.new(0, 510, 0, 16)
...
--==================================================
-- КНОПКИ ВЫБОРА ЦВЕТА (без ползунка)
--==================================================
...
createColorButton("", greenColor, 10, 105)
createColorButton("", yellowColor, 80, 105)
createColorButton("", blueColor, 150, 105)
...
local function fire()
    ...
    fireCooldown = true
    task.delay(FIRE_COOLDOWN_TIME, function()
        fireCooldown = false
    end)
...
--==================================================
-- ГОРЯЧИЕ КЛАВИШИ (Q, ESC)
--==================================================

UserInputService.InputBegan:Connect(function(input, processed)
    if processed then return end

    -- Q для сброса порталов
    ...
    -- Esc для отмены выбора точки (только в режиме POINT)
    if currentMode == "POINT" and pointSelecting then
        if input.KeyCode == Enum.KeyCode.Escape then
            clearPointSelection()
            hud.Text = "SELECTION CANCELLED"
            updateUI()
        end
    end
end)
